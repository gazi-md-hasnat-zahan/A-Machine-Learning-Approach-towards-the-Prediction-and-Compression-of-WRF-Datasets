...
# # result=get_neighbors(np.column_stack((X_reduced, y)),26)
...
for i in range(testfilescount):
    ...
    for file_name in os.listdir(file_path):
        ...
        #     print('X_reduced shape: ' , X_reduced.shape)
            ...
            print('calling get_neighbour function...')
            result=get_neighbors(X_reduced,1,2)
            #np.savetxt("final_pca_reduced_data.csv", result, delimiter=",")

            X_test = result.reshape(699 * 639, 2 * 3 * 3)
            # print(X_train[100:101, :])
            ...
            originalfigname = file_name + 'original_PLS.png'
            preditedfigname = file_name + 'predicted_PLS.png'
...
print("PLS average MSE loss",np.mean(final_mse))
print("PLS average MAE",np.mean(final_mae))
print("PLS average r2 score",np.mean(final_r2))
            
final_r2.append(np.mean(final_r2))
final_mse.append(np.mean(final_mse))
final_mae.append(np.mean(final_mae))
            
np.savetxt("PLS Loss(MSE).txt", final_mse,delimiter=",")
np.savetxt("PLS MAE.txt", final_mae, delimiter=",")
np.savetxt("PLS r2.txt", final_r2, delimiter=",")       
...
#     # create model
#     pls = PLSRegression()
#     pls.fit(X_train[train], Y_train[train])
#     Y_pred=pls.predict(X_train[test])
...
# np.savetxt("PLS_Average CV Loss(MSE).txt", cvscores,delimiter=",")
# np.savetxt("PLS_Average CV MAE.txt", cvscores2, delimiter=",")
# np.savetxt("PLS_Average CV r2_score.txt", cvscores1, delimiter=",")
...
# plt.xlabel('Model')
# plt.show()
# plt.savefig('CVPlot_MSE-PLSModel.png')
...
# plt.xlabel('Model')
# plt.show()
# plt.savefig('CVPlot_MAE-PLSmodel.png')
...
# plt.xlabel('Model')
# plt.show()
# plt.savefig('CVPlot_R2score-PLSModel.png')
